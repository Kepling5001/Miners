...
echo "========================================================"
echo "=== CMP 170HX — Compute-Only All-GPU Unlock v4      ==="
echo "=== Auto-detect build with holder-race recovery      ==="
echo "========================================================"
...
PAYLOAD="${WORKDIR}/payload_compute_only_v4.bin"
PATCHED_GSP="${WORKDIR}/gsp_patched_compute_only_v4.bin"
RUN_GSP_BACKUP="${WORKDIR}/gsp_tu10x.${DRIVER_VERSION}.pre_unlock.bin"
STATE_FILE="${WORKDIR}/post_reset_state_v4.tsv"
LOG_FILE="${WORKDIR}/compute_unlock_v4_$(date +%Y%m%d_%H%M%S).log"
...
main() {
    ...
    read -r -p "Type UNLOCK-ALL-V4 to continue: " answer
    [ "$answer" = "UNLOCK-ALL-V4" ] || die "Cancelled."
    ...
    echo
    echo "========================================================"
    echo "SUCCESS: compute unlock v4 validated on ${#GPUS[@]} GPU(s)"

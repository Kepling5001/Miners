...
echo "========================================================"
echo "=== CMP 170HX — Compute-Only All-GPU Unlock v8.3    ==="
echo "=== nvtool isolation, safe staged CUDA validation           ==="
echo "========================================================"
...
PAYLOAD="${WORKDIR}/payload_compute_only_v8_3.bin"
PATCHED_GSP="${WORKDIR}/gsp_patched_compute_only_v8_3.bin"
RUN_GSP_BACKUP="${WORKDIR}/gsp_tu10x.${DRIVER_VERSION}.pre_unlock.bin"
STATE_FILE="${WORKDIR}/post_reset_state_v8_3.tsv"
LOG_FILE="${WORKDIR}/compute_unlock_v8_3_$(date +%Y%m%d_%H%M%S).log"
INITIAL_STATE_FILE="${WORKDIR}/initial_security_state_v8_3.tsv"
...
#
# v8.3 retains the reset-settle timings. It replaces the unsafe 12-second parallel CUDA timeouts,
# which could interrupt first-context IRQ setup and leave nvidia_uvm permanently busy.
...
FINAL_GPU_MAP="${WORKDIR}/final_gpu_map_v8_3.csv"
...
check_fresh_stage1_state() {
    ...
    if [ "${#stale_wpr2[@]}" -gt 0 ] || [ "${#stale_plm[@]}" -gt 0 ]; then
        ...
        die "A warm reboot was insufficient. Fully remove AC power for at least 30 seconds before running v8.3."
    fi
...
wait_for_all_gpus() {
    ...
    local map_dir="${WORKDIR}/final_proc_map_v8_3"
    rm -rf "$map_dir"
    mkdir -p "$map_dir"
    : > "$FINAL_GPU_MAP"
...
main() {
    ...
    echo "Final NVIDIA enumeration uses non-blocking /proc reads; CUDA uses one generous control test, then pairs."
    ...
    read -r -p "Type UNLOCK-ALL-V8-3 to continue: " answer
    [ "$answer" = "UNLOCK-ALL-V8-3" ] || die "Cancelled."
    ...
    echo
    echo "========================================================"
    echo "SUCCESS: compute unlock v8.3 validated on ${#GPUS[@]} GPU(s)"

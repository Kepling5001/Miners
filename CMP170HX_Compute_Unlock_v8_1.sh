...
echo "========================================================"
echo "=== CMP 170HX — Compute-Only All-GPU Unlock v8.1    ==="
echo "=== nvtool isolation, per-GPU init probes, fast CUDA checks           ==="
echo "========================================================"
...
PAYLOAD="${WORKDIR}/payload_compute_only_v8_1.bin"
PATCHED_GSP="${WORKDIR}/gsp_patched_compute_only_v8_1.bin"
RUN_GSP_BACKUP="${WORKDIR}/gsp_tu10x.${DRIVER_VERSION}.pre_unlock.bin"
STATE_FILE="${WORKDIR}/post_reset_state_v8_1.tsv"
LOG_FILE="${WORKDIR}/compute_unlock_v8_1_$(date +%Y%m%d_%H%M%S).log"
INITIAL_STATE_FILE="${WORKDIR}/initial_security_state_v8_1.tsv"
...
RESTORE_NVTOOL_ON_SUCCESS="${RESTORE_NVTOOL_ON_SUCCESS:-0}"
...
#
# v8.1 retains the reset-settle timings. It fixes v8's global nvidia-smi timeout loop,
# whose one hung GPU could report 0/12 and make a nominal 240-second check run far longer.
...
# Final driver initialization is checked per GPU. A single hung GPU can no
# longer block one global nvidia-smi query or make the displayed elapsed time
# inaccurate. Only GPUs still pending are retried.
FINAL_INIT_ROUNDS="${FINAL_INIT_ROUNDS:-4}"
FINAL_INIT_PROBE_TIMEOUT="${FINAL_INIT_PROBE_TIMEOUT:-8}"
FINAL_INIT_PARALLELISM="${FINAL_INIT_PARALLELISM:-6}"
FINAL_INIT_ROUND_WAIT="${FINAL_INIT_ROUND_WAIT:-5}"
FINAL_GPU_MAP="${WORKDIR}/final_gpu_map_v8_1.csv"

# Fast CUDA validation: GPUs are checked in small parallel batches. There are
# at most two rounds. UVM is recycled only when failures are not accompanied by
# GSP timeout/reset-required errors.
CUDA_TEST_TIMEOUT="${CUDA_TEST_TIMEOUT:-12}"
CUDA_TEST_PARALLELISM="${CUDA_TEST_PARALLELISM:-4}"
CUDA_RETRY_WAIT="${CUDA_RETRY_WAIT:-5}"
UVM_RECYCLE_WAIT="${UVM_RECYCLE_WAIT:-10}"
ENABLE_UVM_RECYCLE="${ENABLE_UVM_RECYCLE:-1}"
CUDA_SMOKE_SRC="${WORKDIR}/cuda_smoke_v8_1.cu"
CUDA_SMOKE_BIN="${WORKDIR}/cuda_smoke_v8_1"
...
restore_nvtool() {
    if [ "$NVTOOL_SUPPRESSED_BY_SCRIPT" -eq 1 ]; then
...
check_fresh_stage1_state() {
    ...
    if [ "${#stale_wpr2[@]}" -gt 0 ] || [ "${#stale_plm[@]}" -gt 0 ]; then
        ...
        die "A warm reboot was insufficient. Fully remove AC power for at least 30 seconds before running v8."
    fi
...
preflight() {
    ...
    for cmd in python3 lspci nvidia-smi modprobe modinfo fuser timeout sort comm awk sed grep \
               sha256sum cmp dmesg tee pgrep pkill killall systemctl readlink lsmod ps seq mount umount mountpoint findmnt screen; do
        require_command "$cmd"
    done
...
wait_for_all_gpus() {
    log "Checking final NVIDIA initialization with bounded per-GPU probes..."
    echo "  rounds=${FINAL_INIT_ROUNDS}, timeout=${FINAL_INIT_PROBE_TIMEOUT}s, parallelism=${FINAL_INIT_PARALLELISM}"

    local probe_dir="${WORKDIR}/final_init_probes_v8_1"
    rm -rf "$probe_dir"
    mkdir -p "$probe_dir"
    : > "$FINAL_GPU_MAP"

    local -a pending=("${GPUS[@]}")
    local round start end i j gpu logfile errfile line
    local index raw_bdf uuid normalized_bdf driver
    local -a pids=() batch_gpus=() batch_logs=() batch_errs=()

    for ((round=1; round<=FINAL_INIT_ROUNDS; round++)); do
        log "Final NVIDIA initialization round ${round}/${FINAL_INIT_ROUNDS} for ${#pending[@]} pending GPU(s)..."

        # Ask the PCI core to reprobe only cards that are unexpectedly unbound.
        ...
        for ((start=0; start<${#pending[@]}; start+=FINAL_INIT_PARALLELISM)); do
            end=$((start + FINAL_INIT_PARALLELISM))
            [ "$end" -le "${#pending[@]}" ] || end="${#pending[@]}"
            ...
            for ((i=start; i<end; i++)); do
                gpu="${pending[$i]}"
                logfile="${probe_dir}/${gpu//[:.]/_}.csv"
                errfile="${probe_dir}/${gpu//[:.]/_}.err"
                : > "$logfile"
                : > "$errfile"

                # Group-level stderr redirection suppresses Bash's noisy
                # "Killed" pipeline message when timeout has to SIGKILL a
                # wedged nvidia-smi process.
                (
                    timeout -s TERM -k 1 "$FINAL_INIT_PROBE_TIMEOUT" \
                        nvidia-smi -i "$gpu" \
            ...
            for ((j=0; j<${#pids[@]}; j++)); do
                ...
                errfile="${batch_errs[$j]}"

                if wait "${pids[$j]}"; then
                    line="$(head -n 1 "$logfile" 2>/dev/null || true)"
                    index=""
                    raw_bdf=""
                    uuid=""
                    IFS=',' read -r index raw_bdf uuid <<< "$line"
                    index="$(echo "$index" | xargs)"
                    raw_bdf="$(echo "$raw_bdf" | xargs)"
                    uuid="$(echo "$uuid" | xargs)"
                    normalized_bdf="$(printf '%s\n' "$raw_bdf" | normalize_bdfs)"

                    if [ "$normalized_bdf" = "$gpu" ] && [ -n "$uuid" ]; then
                        printf '%s,%s,%s\n' "$index" "$raw_bdf" "$uuid" > "$logfile"
                        echo "  $gpu: NVIDIA ready (index $index)"
                        continue
                    fi
                fi
                ...
                FINAL_PROBE_FAILED+=("$gpu")
            done
        done

        local ready_count=$(( ${#GPUS[@]} - ${#FINAL_PROBE_FAILED[@]} ))
        echo "  Result: ${ready_count}/${#GPUS[@]} GPUs ready"

        if [ "${#FINAL_PROBE_FAILED[@]}" -eq 0 ]; then
            : > "$FINAL_GPU_MAP"
            for gpu in "${GPUS[@]}"; do
                logfile="${probe_dir}/${gpu//[:.]/_}.csv"
                [ -s "$logfile" ] || {
                    echo "Missing saved NVIDIA mapping for $gpu"
                    return 1
                }
                cat "$logfile" >> "$FINAL_GPU_MAP"
            done
            return 0
        fi
        ...
        if fatal_cuda_error_present; then
            echo "A GSP timeout or reset-required error appeared during final initialization."
            echo "Stopping early instead of repeating probes against reset-required GPUs."
            break
        fi

        pending=("${FINAL_PROBE_FAILED[@]}")
    ...
    echo
    echo "GPUs that never completed an isolated NVIDIA query:"
    printf '  %s\n' "${FINAL_PROBE_FAILED[@]}"
    return 1
}
...
build_cuda_smoke_test() {
    ...
    command -v nvcc >/dev/null 2>&1 ||
        die "nvcc is required for v8.1 CUDA validation, or set SKIP_CUDA_SMOKE=1."
...
recycle_uvm_once() {
    log "Recycling nvidia_uvm once before the final fast CUDA retry..."
...
fatal_cuda_error_present() {
    dmesg | grep -qiE \
        'Xid \(PCI:.*\): (119|154)|GSP Timeout|NV_ERR_RESET_REQUIRED|Reset required'
}

run_cuda_parallel_round() {
    local round="$1"
    shift
    local -a targets=("$@")
    CUDA_PENDING=()

    log "Fast CUDA round $round for ${#targets[@]} GPU(s), parallelism=${CUDA_TEST_PARALLELISM}..."

    local start end i j gpu test_uuid test_index logfile
    local -a pids=() batch_gpus=() batch_logs=()
    ...
    for ((start=0; start<${#targets[@]}; start+=CUDA_TEST_PARALLELISM)); do
        ...
        for ((i=start; i<end; i++)); do
            ...
            logfile="${WORKDIR}/cuda_v8_1_round${round}_${gpu//[:.]/_}.log"
            : > "$logfile"

            (
                CUDA_DEVICE_ORDER=PCI_BUS_ID \
                CUDA_VISIBLE_DEVICES="$test_uuid" \
                timeout -k 2 "$CUDA_TEST_TIMEOUT" "$CUDA_SMOKE_BIN"
            ) > "$logfile" 2>&1 &
...
cuda_smoke_test() {
    log "Running fast isolated per-GPU CUDA validation..."
    ...
    run_cuda_parallel_round 1 "${GPUS[@]}"
    if [ "${#CUDA_PENDING[@]}" -eq 0 ]; then
        log "Every expected GPU passed CUDA validation in the first fast round."
        return 0
    fi

    echo "CUDA failed initially on:"
    printf '  %s\n' "${CUDA_PENDING[@]}"

    if fatal_cuda_error_present; then
        echo
        echo "A GSP timeout or reset-required error is already present."
        echo "Skipping pointless UVM retries."
        return 1
    fi

    local -a retry_targets=("${CUDA_PENDING[@]}")

    if [ "$ENABLE_UVM_RECYCLE" = "1" ]; then
        recycle_uvm_once || true
    else
        log "Waiting ${CUDA_RETRY_WAIT}s before the one final retry..."
        sleep "$CUDA_RETRY_WAIT"
    fi

    run_cuda_parallel_round 2 "${retry_targets[@]}"
    if [ "${#CUDA_PENDING[@]}" -eq 0 ]; then
        log "All remaining GPUs passed the second fast CUDA round."
        return 0
    fi
...
main() {
    ...
    echo "Final NVIDIA initialization uses isolated per-GPU probes; CUDA uses at most two small parallel rounds."
    ...
    read -r -p "Type UNLOCK-ALL-V8-1 to continue: " answer
    [ "$answer" = "UNLOCK-ALL-V8-1" ] || die "Cancelled."
    ...
    cuda_smoke_test ||
        die "One or more visible GPUs failed the fast CUDA smoke test."
    ...
    if [ "$RESTORE_NVTOOL_ON_SUCCESS" = "1" ]; then
        restore_nvtool || true
    else
        echo
        echo "HiveOS nvtool remains suppressed for the unlocked session."
        echo "Start the miner and confirm stable hashing before restoring it."
        print_nvtool_restore_command
    fi

    echo
    echo "========================================================"
    echo "SUCCESS: compute unlock v8.1 validated on ${#GPUS[@]} GPU(s)"
    echo "No STRAP or LMR VRAM-capacity writes were used."
    echo "All GPUs passed NVIDIA initialization and fast CUDA validation."
    echo "========================================================"

...
echo "========================================================"
echo "=== CMP 170HX — Compute-Only All-GPU Unlock v8      ==="
echo "=== nvtool isolation, explicit Stage 1 trigger, fast CUDA checks      ==="
echo "========================================================"
...
PAYLOAD="${WORKDIR}/payload_compute_only_v8.bin"
PATCHED_GSP="${WORKDIR}/gsp_patched_compute_only_v8.bin"
RUN_GSP_BACKUP="${WORKDIR}/gsp_tu10x.${DRIVER_VERSION}.pre_unlock.bin"
STATE_FILE="${WORKDIR}/post_reset_state_v8.tsv"
LOG_FILE="${WORKDIR}/compute_unlock_v8_$(date +%Y%m%d_%H%M%S).log"
INITIAL_STATE_FILE="${WORKDIR}/initial_security_state_v8.tsv"
...
#
# v8 retains the longer reset-settle timings while changing only monitoring and validation.
...
CUDA_SMOKE_SRC="${WORKDIR}/cuda_smoke_v8.cu"
CUDA_SMOKE_BIN="${WORKDIR}/cuda_smoke_v8"
...
build_cuda_smoke_test() {
    ...
    command -v nvcc >/dev/null 2>&1 ||
        die "nvcc is required for v8 CUDA validation, or set SKIP_CUDA_SMOKE=1."
...
run_cuda_parallel_round() {
    ...
    for ((start=0; start<${#targets[@]}; start+=CUDA_TEST_PARALLELISM)); do
        ...
        for ((i=start; i<end; i++)); do
            ...
            logfile="${WORKDIR}/cuda_v8_round${round}_${gpu//[:.]/_}.log"
            : > "$logfile"
...
cuda_smoke_test() {
    ...
    local map_file="${WORKDIR}/cuda_device_map_v8.csv"

    timeout -k 2 20 nvidia-smi \
...
main() {
    ...
    echo "CUDA checks use at most two small parallel rounds, not eight serial passes."
    ...
    read -r -p "Type UNLOCK-ALL-V8 to continue: " answer
    [ "$answer" = "UNLOCK-ALL-V8" ] || die "Cancelled."
    ...
    echo
    echo "========================================================"
    echo "SUCCESS: compute unlock v8 validated on ${#GPUS[@]} GPU(s)"

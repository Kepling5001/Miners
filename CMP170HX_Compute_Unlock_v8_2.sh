...
echo "========================================================"
echo "=== CMP 170HX — Compute-Only All-GPU Unlock v8.2    ==="
...
PAYLOAD="${WORKDIR}/payload_compute_only_v8_2.bin"
PATCHED_GSP="${WORKDIR}/gsp_patched_compute_only_v8_2.bin"
RUN_GSP_BACKUP="${WORKDIR}/gsp_tu10x.${DRIVER_VERSION}.pre_unlock.bin"
STATE_FILE="${WORKDIR}/post_reset_state_v8_2.tsv"
LOG_FILE="${WORKDIR}/compute_unlock_v8_2_$(date +%Y%m%d_%H%M%S).log"
INITIAL_STATE_FILE="${WORKDIR}/initial_security_state_v8_2.tsv"
...
#
# v8.2 retains the reset-settle timings. It fixes v8's global nvidia-smi timeout loop,
...
FINAL_GPU_MAP="${WORKDIR}/final_gpu_map_v8_2.csv"
...
CUDA_SMOKE_SRC="${WORKDIR}/cuda_smoke_v8_2.cu"
CUDA_SMOKE_BIN="${WORKDIR}/cuda_smoke_v8_2"
...
wait_for_all_gpus() {
    ...
    local map_dir="${WORKDIR}/final_proc_map_v8_2"
    rm -rf "$map_dir"
    mkdir -p "$map_dir"
    : > "$FINAL_GPU_MAP"
...
build_cuda_smoke_test() {
    ...
    command -v nvcc >/dev/null 2>&1 ||
        die "nvcc is required for v8.2 CUDA validation, or set SKIP_CUDA_SMOKE=1."
...
run_cuda_parallel_round() {
    ...
    local start end i j gpu test_uuid test_index logfile statusfile rc
    local -a pids=() batch_gpus=() batch_logs=() batch_status=()
    ...
    for ((start=0; start<${#targets[@]}; start+=CUDA_TEST_PARALLELISM)); do
        ...
        for ((i=start; i<end; i++)); do
            ...
            logfile="${WORKDIR}/cuda_v8_2_round${round}_${gpu//[:.]/_}.log"
            statusfile="${WORKDIR}/cuda_v8_2_round${round}_${gpu//[:.]/_}.status"
            : > "$logfile"
            rm -f "$statusfile"

            # Keep the background shell alive even when timeout kills a wedged
            # CUDA process. The parent reads an explicit status file, avoiding
            # Bash's noisy asynchronous "Killed" job notifications.
            (
                set +e
                CUDA_DEVICE_ORDER=PCI_BUS_ID \
                CUDA_VISIBLE_DEVICES="$test_uuid" \
                timeout -s TERM -k 2 "$CUDA_TEST_TIMEOUT" "$CUDA_SMOKE_BIN" \
                    > "$logfile" 2>&1
                rc=$?
                printf '%s\n' "$rc" > "$statusfile"
                exit 0
        ...
        for ((j=0; j<${#pids[@]}; j++)); do
            ...
            statusfile="${batch_status[$j]}"

            wait "${pids[$j]}" 2>/dev/null || true
            rc="$(cat "$statusfile" 2>/dev/null || echo 255)"
...
main() {
    ...
    echo "Final NVIDIA enumeration uses non-blocking /proc reads; CUDA uses at most two small parallel rounds."
    ...
    read -r -p "Type UNLOCK-ALL-V8-2 to continue: " answer
    [ "$answer" = "UNLOCK-ALL-V8-2" ] || die "Cancelled."
    ...
    cuda_smoke_test ||
        die "One or more enumerated GPUs failed the fast CUDA smoke test."
    ...
    echo
    echo "========================================================"
    echo "SUCCESS: compute unlock v8.2 validated on ${#GPUS[@]} GPU(s)"
    echo "No STRAP or LMR VRAM-capacity writes were used."
    echo "All GPUs enumerated and passed fast CUDA validation."
    echo "========================================================"
